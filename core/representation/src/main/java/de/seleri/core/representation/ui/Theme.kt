package de.seleri.core.representation.ui
...
fun ImpulseTheme(content: @Composable () -> Unit) {
	...
	val formatierung = Typography(
		displayLarge = TextStyle(
			fontWeight = FontWeight.Bold, fontSize = 60.sp, shadow = Shadow(
				color = Color.Black.copy(alpha = 0.3f), offset = Offset(2f, 2f), blurRadius = 6f
			)
		), displayMedium = TextStyle(
			fontWeight = FontWeight.Bold, fontSize = 30.sp, shadow = Shadow(
				color = Color.Black.copy(alpha = 0.3f), offset = Offset(1f, 1f), blurRadius = 4f
			)
		)
	)

	MaterialTheme(
		colorScheme = farbpalette, typography = formatierung
	) {
		Surface(
			modifier = Modifier.fillMaxSize(), color = farbpalette.background
		) {
			content()
		}
	}
